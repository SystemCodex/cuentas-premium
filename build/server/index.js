...
import { z } from 'zod';
import { disconnectWhatsAppBridge, getWhatsAppBridgeQr, getWhatsAppBridgeStatus, queueWhatsAppNotification, retryFailedWhatsAppOutbox, startWhatsAppBridgeWorker } from './services/whatsappBridge/index.js';
...
async function getProviderPaymentConfig(providerId) {
...
async function handleWhatsAppOutboxFinalFailure(item) {
    ...
    if (!order || !payout)
        return;
    if (order.admin_notification_channel === 'email' || order.admin_notification_channel === 'whatsapp')
        return;
...
app.post('/api/orders', sensitiveLimiter, requireAuth, requireRole('client'), async (req, res, next) => {
    try {
        ...
        if (!providerPaymentConfig)
            return res.status(400).json({ message: 'No hay una configuracion activa de pago al proveedor.' });
